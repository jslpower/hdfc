<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OtherIncomeAdd.aspx.cs"
    Inherits="Web.Fin.OtherIncomeAdd" MasterPageFile="~/MasterPage/Boxy.Master" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="PageBody" runat="server">
            ...
            <tr class="odd">
                ...
                <td bgcolor="#E3F1FC">
                    <input name="txtPrice" type="text" class="inputtext" id="txtPrice" runat="server"
                        valid="required|isMoney" errmsg="请输入收入金额|金额格式不正确" />
                </td>
                <th align="right">
                    收款人：
                </th>
                        ...
                        <asp:Repeater ID="rpFile" runat="server">
                            <ItemTemplate>
                                <span class='upload_filename'><a href='<%#Eval("FilePath") %>' target="_blank">
                                    <%#Eval("FileName")%></a> <a href="javascript:void(0)" onclick="OtherIncomeAdd.RemoveFile(this)"
                                        title='删除附件'>
    ...
    <script type="text/javascript">
        $(function() {
            OtherIncomeAdd.PageInit();
        })
        var OtherIncomeAdd = {
            PageInit: function() {
                $("input[readonly='readonly']").css({ "background-color": "#dadada" });
                $("#<%=btn.ClientID %>").click(function() {
                    if (!OtherIncomeAdd.CheckForm()) {
                        return false;
                    }
                    var url = "/Fin/OtherIncomeAdd.aspx?dotype=" + '<%=Request.QueryString["dotype"]%>' + "&type=save&tid=" + '<%=Request.QueryString["tid"]%>';
                    OtherIncomeAdd.GoAjax(url);
                    return false;
                });
            },
            GoAjax: function(url) {
                OtherIncomeAdd.UnBind();
                $.newAjax({
                    type: "post",
                    cache: false,
                    url: url,
