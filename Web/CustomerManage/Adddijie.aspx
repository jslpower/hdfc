<%@ Page Language="C#" MasterPageFile="~/MasterPage/Boxy.Master" AutoEventWireup="true" CodeBehind="Adddijie.aspx.cs" Inherits="Web.CustomerManage.Adddijie" Title="地接社" %>
...
<form id="form1" runat="server">
    ...
    <script type="text/javascript">
        $(function(){
            AddDijiePage.BindBtn();
        })
        var AddDijiePage={
            BindBtn:function(){
                $("#<%=btn.ClientID %>").click(function(){
                    var url="/CustomerManage/Adddijie.aspx?dotype=save&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+'<%=Request.QueryString["giftid"] %>';
                    if(!AddDijiePage.CheckForm()){
                        return ;
                    }
                    $(this).html("正在保存");
                    AddDijiePage.GoAjax(url);
                })
                $(".update").click(function(){
                    var giftid=$(this).closest("tr").attr("data-id");
                    var url="/CustomerManage/Adddijie.aspx?dotype=show&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+giftid;
                    window.location.href=url;
                })
                $(".delete").click(function(){
                        var giftid=$(this).closest("tr").attr("data-id");
                        var url="/CustomerManage/Adddijie.aspx?dotype=delete&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+giftid;
            ...
            GoAjax: function(url) {
                ...
                $.newAjax({
                    ...
                    success: function(ret) {
                        if (ret.result == "1") {
                            parent.tableToolbar._showMsg(ret.msg, function() { location.href = "/CustomerManage/Adddijie.aspx?dotype=add&contactId="+'<%=Request.QueryString["contactId"] %>'; });
                        }
                        else {
