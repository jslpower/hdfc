<%@ Page Language="C#" MasterPageFile="~/MasterPage/Boxy.Master" AutoEventWireup="true" CodeBehind="AddDaoyou.aspx.cs" Inherits="Web.CustomerManage.AddDaoyou" Title="导游" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="PageBody" runat="server">
 <form id="form1" runat="server">
    ...
    <script type="text/javascript">
        ...
        var AddGiftPage={
            BindBtn:function(){
                $("#<%=btn.ClientID %>").unbind("click").click(function() {
                    var url="/CustomerManage/AddDaoyou.aspx?dotype=save&guideid="+'<%=Request.QueryString["guideid"] %>'+"&giftid="+'<%=Request.QueryString["giftid"] %>';
                    if(!AddGiftPage.CheckForm()){
                        return ;
                    }
                    $(this).html("正在保存");
                    AddGiftPage.GoAjax(url);
                })
                $(".update").click(function(){
                    var giftid=$(this).closest("tr").attr("data-id");
                    var url="/CustomerManage/AddDaoyou.aspx?dotype=show&guideid="+'<%=Request.QueryString["guideid"] %>'+"&giftid="+giftid;
                    window.location.href=url;
                })
                $(".delete").click(function(){
                        var giftid=$(this).closest("tr").attr("data-id");
                        var url="/CustomerManage/AddDaoyou.aspx?dotype=delete&guideid="+'<%=Request.QueryString["guideid"] %>'+"&giftid="+giftid;
            ...
            GoAjax: function(url) {
                ...
                $.newAjax({
                    ...
                    success: function(ret) {
                        if (ret.result == "1") {
                            parent.tableToolbar._showMsg(ret.msg, function() { location.href = "/CustomerManage/AddDaoyou.aspx?dotype=add&guideid="+'<%=Request.QueryString["guideid"] %>'; });
                        }
                        else {
