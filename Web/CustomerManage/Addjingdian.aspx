<%@ Page Language="C#" MasterPageFile="~/MasterPage/Boxy.Master" AutoEventWireup="true" CodeBehind="Addjingdian.aspx.cs" Inherits="Web.CustomerManage.Addjingdian" Title="景点" %>
...
<form id="form1" runat="server">
    ...
    <script type="text/javascript">
        $(function(){
            AddJingdianPage.BindBtn();
        })
        var AddJingdianPage={
            BindBtn:function(){
                $("#<%=btn.ClientID %>").click(function(){
                    var url="/CustomerManage/Addjingdian.aspx?dotype=save&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+'<%=Request.QueryString["giftid"] %>';
                    if(!AddJingdianPage.CheckForm()){
                        return ;
                    }
                    $(this).html("正在保存");
                    AddJingdianPage.GoAjax(url);
                })
                $(".update").click(function(){
                    var giftid=$(this).closest("tr").attr("data-id");
                    var url="/CustomerManage/Addjingdian.aspx?dotype=show&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+giftid;
                    window.location.href=url;
                })
                $(".delete").click(function(){
                        var giftid=$(this).closest("tr").attr("data-id");
                        var url="/CustomerManage/Addjingdian.aspx?dotype=delete&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+giftid;
            ...
            GoAjax: function(url) {
                ...
                $.newAjax({
                    ...
                    success: function(ret) {
                        if (ret.result == "1") {
                            parent.tableToolbar._showMsg(ret.msg, function() { location.href = "/CustomerManage/Addjingdian.aspx?dotype=add&contactId="+'<%=Request.QueryString["contactId"] %>'; });
                        }
                        else {
