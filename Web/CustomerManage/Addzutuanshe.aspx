<%@ Page Language="C#" MasterPageFile="~/MasterPage/Boxy.Master" AutoEventWireup="true" CodeBehind="Addzutuanshe.aspx.cs" Inherits="Web.CustomerManage.Addzutuanshe" Title="组团社生日" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="PageBody" runat="server">
 <form id="form1" runat="server">
    ...
    <script type="text/javascript">
        $(function(){
            AddZutuanPage.BindBtn();
        })
        var AddZutuanPage={
            BindBtn:function(){
                $("#<%=btn.ClientID %>").click(function(){
                    var url="/CustomerManage/Addzutuanshe.aspx?dotype=save&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+'<%=Request.QueryString["giftid"] %>';
                    if(!AddZutuanPage.CheckForm()){
                        return ;
                    }
                    $(this).html("正在保存");
                    AddZutuanPage.GoAjax(url);
                })
                $(".update").click(function(){
                    var giftid=$(this).closest("tr").attr("data-id");
                    var url="/CustomerManage/Addzutuanshe.aspx?dotype=show&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+giftid;
                    window.location.href=url;
                })
                $(".delete").click(function(){
                        var giftid=$(this).closest("tr").attr("data-id");
                        var url="/CustomerManage/Addzutuanshe.aspx?dotype=delete&contactId="+'<%=Request.QueryString["contactId"] %>'+"&giftid="+giftid;
            ...
            GoAjax: function(url) {
                ...
                $.newAjax({
                    ...
                    success: function(ret) {
                        if (ret.result == "1") {
                            parent.tableToolbar._showMsg(ret.msg, function() { location.href = "/CustomerManage/Addzutuanshe.aspx?dotype=add&contactId="+'<%=Request.QueryString["contactId"] %>'; });
                        }
                        else {
